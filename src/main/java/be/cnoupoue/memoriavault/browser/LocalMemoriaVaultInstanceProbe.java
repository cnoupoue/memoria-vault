...
public class LocalMemoriaVaultInstanceProbe {
  ...
  public LocalMemoriaVaultInstanceProbe() {
    this(HttpClient.newBuilder().connectTimeout(REQUEST_TIMEOUT).build());
  }

  LocalMemoriaVaultInstanceProbe(HttpClient httpClient) {
  ...
  public InstanceProbeResult probe(URI appUrl) {
    try {
      ...
      return InstanceProbeResult.HEALTHY_MEMORIA_VAULT;
