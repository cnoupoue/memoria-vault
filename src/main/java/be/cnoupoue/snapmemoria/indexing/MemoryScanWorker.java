...
public class MemoryScanWorker {
    ...
    public void execute(String scanJobId) {
        MemoryScanJob scanJob = memoryScanJobRepository.findById(scanJobId)
                .orElseThrow(() -> new IllegalStateException("Scan job not found."));

        try {
            ScanProgress finalProgress = memorySourceScanner.scan(
                    scanJob.getSourceId(),
                    progress -> updateProgress(scanJobId, progress)
            );
    ...
    private void markFailed(
        ...
        scanJob.markFailed(
                message == null ? "The scan failed unexpectedly." : message,
