...
class LocalSnapmemoriaInstanceProbeTest {

  private final LocalSnapmemoriaInstanceProbe instanceProbe = new LocalSnapmemoriaInstanceProbe();

  @Test
  void detectsExistingHealthySnapmemoriaInstance() throws IOException {
    try (TestHttpServer server =
      ...
      assertThat(result).isEqualTo(InstanceProbeResult.HEALTHY_SNAPMEMORIA);
    }
  }

  @Test
  void doesNotTreatUnrelatedServiceAsSnapmemoria() throws IOException {
    try (TestHttpServer server =
        TestHttpServer.create()
            .responding("/actuator/health", 200, "{\"status\":\"UP\"}")
  ...
  @Test
  void doesNotTreatInvalidHealthResponseAsSnapmemoria() throws IOException {
    try (TestHttpServer server =
        TestHttpServer.create()
