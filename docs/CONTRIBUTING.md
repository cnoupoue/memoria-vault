# Contributing to SnapMemoria

Thank you for contributing to SnapMemoria.

SnapMemoria is a local-first application for browsing exported Snapchat Memories. The project is designed to keep original media files outside the repository while indexing metadata and generating local cached previews.
...
SnapMemoria uses Conventional Commits.
...
5. Do not commit personal Snapchat exports, SQLite databases, thumbnail caches, `.env` files, or local source paths.
...
Original Snapchat files must remain outside the project directory and outside Git.
